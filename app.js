...
const humanExpressions = [
    ...
    { img: './img/angry.jpg', text: 'Estou com fome' },
...
main.addEventListener('click', (event) => {
    ...
    const clickdeElementTextMustSpoken = ['IMG', 'P'].some(elementName => 
        clickedElement.tagName.toLowerCase() === elementName.toLowerCase())

    if (clickdeElementTextMustSpoken) {
...
const setPTBRVoices = (voices) => {
    ...
    const microsoftVoice2 = voices.find(voice =>
       voice.name === 'Microsoft Daniel - Portuguese (Brazil)');

    if(microsoftVoice) {
        setUtteranceVoice(microsoftVoice)
    } else if(microsoftVoice2) {
        setUtteranceVoice(microsoftVoice2)
    }
}
